...
window.MargoTipsParser = {
    ...
    createCmpSegment: function(section1, cmp, headPart, i, configSections) {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            section1.create(11, i, sectionStr, {
                cssClass: cmpSectionClass
            });
        }
        ...
        if (sectionData11) {
            ...
            section1.create(11, cmpStatName, cmpText, {
                cssClass: cmpTextClass
            });
        }
    },
    getTip: function(i, cmp) {
        ...
        if (isset(i.name)) {
            ...
            headPart.create(1, actualHeadStatName, actualHeadStatText, {
                cssClass: actualHeadStatName
            });
        }
        ...
        if (isset(i.cl)) {
            ...
            headPart.create(2, actualHeadStatName, actualHeadStatText, {
                cssClass: actualHeadStatName
            });
        }
        ...
        if (isset(i.getBuildsWithThisItem) && i.getBuildsWithThisItem() != null) {
            ...
            headPart.create(3, actualHeadStatName, actualHeadStatText, {
                cssClass: actualHeadStatName
            });
        }
        ...
        if (valueStatExist) {
            ...
            valueStatText = _t('item_value %val%', {
                '%val%': '<span class="val">' + round(i.pr, (i.pr < 10000 ? 10 : 2))
            }) + '</span>' + getCurrencyIcon(i.prc)

            section1.create(9, 'item_value', valueStatText, {
                cssClass: "value-item"
            });
        } else {
        ...
        for (var k in st) {
            ...
            switch (actualStatName) {
                ...
                case 'rarity':
                    ...
                    headPart.addOptions(1, {
                        cssClass: [actualStatVal]
                    });
                ...
                case 'enhancement_upgrade_lvl':
                    ...
                    headPart.create(0, actualStatName, '', {
                        cssClass: clToAdd
                    });
                    break;
                case 'bonus':
...
*/
                ...
                default:
                    if (actualStatName != '') {
                        ...
                        section.create(3, actualStatName, _t('unknown_stat %val%', {
                            '%val%': actualStatName
                        }));

                        console.log(_t('unknown_stat %val%', {
                            '%val%': actualStatName
                        }));
                    }
...
window.MargoTipsParser.statsParser = {
    ...
    enhancement_refund: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (actualStatVal > 1) {
            statText += '<br>' + _t2('bonus_' + actualStatName + '_amount', {
                '%val%': actualStatVal
            });
        }
    ...
    quest_expbon: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (actualStatVal > 0) {
            statText = _t2('bonus_' + actualStatName + ' higher %val%', {
                '%val%': actualStatVal + MargoTipsParser.addUnit('%')
            }) + cmpStr;
        } else {
            statText = _t2('bonus_' + actualStatName + ' lower %val%', {
                '%val%': actualStatVal + MargoTipsParser.addUnit('%')
            }) + cmpStr;
        }
    ...
    expaddlvl: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        let statText = _t2('bonus_' + actualStatName + ' %val%', {
            '%val%': b[1],
            '%val2%': lvl
        });
    ...
    bag: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        let statText = _t2('bonus_bag %val%', {
            '%val%': actualStatVal
        }, 'newOrder') + posfix;
    ...
    btype: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        let statText = _t('bonus_btype %val%', {
            '%val%': typeTranslations.join(', ')
        });
    ...
    outfit: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (b[0] < 1) {
            perm = true;
        } else if (b[0] < 99) tm = _t('amount_min %amount%', {
            '%amount%': b[0]
        }, 'newOrder'); //" minut"
        else if (b[0] < 300) tm = _t('amount_hrs2 %amount%', {
            '%amount%': round(b[0] / 60)
        }, 'newOrder'); //" godziny"
        else tm = _t('amount_hrs %amount%', {
            '%amount%': round(b[0] / 60)
        }, 'newOrder'); //" godzin"
        ...
        if (perm) {
            ...
            statText = _t('outfit_change_for %time%', {
                '%time%': tm
            }) + changeOn;
        }
    ...
    battlestats: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (MargoTipsParser.isInterface()) {
            ...
            statText = _t('battlestats', {
                '%val%': val
            });
        } else if (isset(MargoSkillsParser)) {
            statText = _t('battlestats', {
                '%val%': MargoSkillsParser.getStatsForTips(actualStatVal).join('<br/>')
            });
        }
    ...
    timelimit: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (b[0] < 1) statText = _t2('timelimit_can be used %val% sec', {
            '%val%': b[0]
        });
        else if (b[0] == 1) statText = _t2('timelimit_can be used %val% minutes2', {
            '%val%': 1
        }, 'newOrder');
        else if (b[0] < 5) statText = _t2('timelimit_can be used %val% minutes2', {
            '%val%': b[0]
        }, 'newOrder');
        else statText = _t2('timelimit_can be used %val% minutes2', {
            '%val%': b[0]
        }, 'newOrder');
        ...
        if (isset(b[1])) {
            var isCounter = b[1] - unix_time() >= 0;
            if (!isCounter) statText += '<br>' + _t('timelimit_readyToUse_now');
            else statText += '<br>' + _t2('timelimit_readyToUse_in', {
                '%val%': calculateDiffFull(b[1], unix_time(), false)
            });
        }
    ...
    expires: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (actualStatVal - unix_time() < 0) {
            ...
            statText = _t2('valid_expires %date%', {
                '%val%': calculateDiffFull(actualStatVal, unix_time())
            });
            // section.create(1, actualStatName, statText, {cssClass: "item-expired"});
            options.cssClass = "item-expired";
        }
    ...
    ttl: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (itemData.cl == 25 && (itemData.loc == 't' || itemData.loc == 'n' || itemData.loc == 'o' || itemData.loc == 'r' || itemData.loc == 'd' || itemData.loc == 'c' || (itemData.loc == 'g' && (itemData.st == 0 || itemData.st == 9)) || (itemData.loc == 's' && itemData.st == 0))) {
            ...
            statText = _t2('ttl1 %date%', {
                '%val%': actualStatVal
            }, 'newOrder');
        } else {

            statText = _t2('ttl2 %date%', {
                '%val%': actualStatVal
            }, 'newOrder');
        }
    ...
    amount: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (itemData.st != 10) {
            if (cursed) {
                statText = _t2('cursed_amount %val%', {
                    '%val%': valAndTag
                }) + cmpStr
            } else {
                statText = _t2('amount %val% %split%', {
                    '%val%': valAndTag + cmpStr,
                    '%split%': ''
                });
            }
    ...
    capacity: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        if (itemData.st != 10) {
            ...
            let statText = _t2('capacity %val%', {
                '%val%': formNumberToNumbersGroup(actualStatVal)
            });
    ...
    npc_expbon: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData, allStats, missStats) {
        ...
        if (actualStatVal > 0) {
            ...
            statText = _t2('npx_expbon higher %amount%', {
                '%amount%': Math.abs(actualStatVal) + MargoTipsParser.addUnit('%')
            }, 'newOrder')
            // section.create(1, actualStatName, statText);

        } else if (actualStatVal < 0) {

            statText = _t2('npx_expbon lower %amount%', {
                '%amount%': Math.abs(actualStatVal) + MargoTipsParser.addUnit('%')
            }, 'newOrder');
            // section.create(1, actualStatName, statText);
    ...
    leczy: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr) {
        ...
        if (actualStatVal > 0) {
            ...
            if (actualStatVal.split(':').length > 1) {
                ...
                newVal = _t('start') + ' ' + formNumberToNumbersGroup(t[0]) + ' ' + _t('stop') + ' ' + formNumberToNumbersGroup(t[1]);
                statText = _t2('bonus_truje2 %val%', {
                    '%val%': newVal
                }) + cmpStr;
            } else {
                statText = _t2('bonus_truje %val%', {
                    '%val%': formNumberToNumbersGroup(Math.abs(actualStatVal))
                }) + cmpStr;
            }
        }
    ...
    legbon_test: function(sectionIndex, actualStatName, actualStatVal, options) {
        ...
        switch (b[0]) {
            ...
            case 'puncture':
            case 'frenzy':
                statText += 'TEST: ' + _t('legbon_' + b[0], {
                    '%val%': b[1]
                });
                break;
            default:
                statText += _t2('legbon_undefined %val%', {
                    '%val%': b[0]
                });
                break;
        }
    ...
    expire_duration: function(sectionIndex, actualStatName, actualStatVal, options) {
        ...
        let statText = _t2('expire_duration %time%', {
            '%time%': t
        });
    ...
    action: function(sectionIndex, actualStatName, actualStatVal, options) {
        ...
        switch (c[0]) {
            ...
            case 'fatigue':
                var f_val = parseInt(c[1]);
                if (f_val > 0) {
                    statText = _t2('fatigue_positive %val%', {
                        '%val%': Math.abs(f_val)
                    }, 'newOrder');
                } else {
                    statText = _t2('fatigue_negative %val%', {
                        '%val%': Math.abs(f_val)
                    }, 'newOrder');
                }
                break;
            case 'fightperheal':
                if (c.length == 2) {
                    ...
                    statText = _t2('fightperheal %amount%', {
                        '%amount%': c[1] + MargoTipsParser.addUnit('%')
                    }, 'newOrder');
                } else if (c.length == 3) {
    ...
    lowreq: function(sectionIndex, actualStatName, actualStatVal, options) {
        let statText = '* ' + _t2('type_lower_req %val%', {
            '%val%': actualStatVal
        }) + ' *';
    ...
    dmg: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData) {
        ...
        const statText = _t2(translation, {
            '%val%': MargoTipsParser.getRanges2(actualStatVal)
        }) + cmpStr1 + cmpStr2;
    ...
    perheal: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData) {
        ...
        if (actualStatVal > 0) {
            ...
            statText = _t2('bonus_perheal %val%', {
                '%val%': actualStatVal + MargoTipsParser.addUnit('%')
            }, 'newOrder');
        } else {

            statText = _t2('bonus_perheal_minus %val%', {
                '%val%': Math.abs(actualStatVal) + MargoTipsParser.addUnit('%')
            });
        }
    ...
    wanted_change: function(sectionIndex, actualStatName, actualStatVal, options, cmpStr, cmp, itemData) {
        ...
        if (actualStatVal >= 0) {
            statText = _t2(`${actualStatName} higher %val%`, {
                '%val%': Math.abs(actualStatVal)
            });
        } else {
            statText = _t2(`${actualStatName} lower %val%`, {
                '%val%': Math.abs(actualStatVal)
            });
        }
...
window.MargoTipsParser.statsData = {
    loot: {
        sectionIndex: 6,
        dataFunc: MargoTipsParser.statsParser.loot,
        cssClass: "looter",
        htmlTag: "i"
    },
    rarity: {
        sectionIndex: 1,
        dataFunc: MargoTipsParser.statsParser.rarity
    },
    enhancement_refund: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.enhancement_refund
    },
    quest_expbon: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.quest_expbon,
        eqStatClass: true
    },
    expaddlvl: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.expaddlvl
    },
    gold: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.gold,
        eqStatClass: true
    },
    bag: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.bag
    },
    btype: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.btype
    },
    outfit: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.outfit
    },
    battlestats: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.battlestats
    },
    timelimit: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.timelimit
    },
    expires: {
        sectionIndex: 1,
        dataFunc: MargoTipsParser.statsParser.expires
    },
    ttl: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.ttl,
        eqStatClass: true
    },
    amount: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.amount,
        eqStatClass: true
    },
    cansplit: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.cansplit
    },
    capacity: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.capacity
    },
    opis: {
        sectionIndex: 7,
        dataFunc: MargoTipsParser.statsParser.opis
    },
    npc_expbon: {
        sectionIndex: 1,
        dataFunc: MargoTipsParser.statsParser.npc_expbon
    },
    lvl: {
        sectionIndex: 9,
        dataFunc: MargoTipsParser.statsParser.lvl,
        cssClass: "level-required"
    },
    reqp: {
        sectionIndex: 9,
        dataFunc: MargoTipsParser.statsParser.reqp,
        cssClass: "prof-icons-holder"
    },
    target_class: {
        sectionIndex: 9,
        dataFunc: MargoTipsParser.statsParser.target_class,
        cssClass: "target-class"
    },
    pet: {
        sectionIndex: 2,
        dataFunc: MargoTipsParser.statsParser.pet,
        cssClass: "pet-tasks"
    },
    nodepoclan: {
        sectionIndex: 8,
        dataFunc: MargoTipsParser.statsParser.nodepoclan
    },
    nodepo: {
        sectionIndex: 8,
        dataFunc: MargoTipsParser.statsParser.nodepo
    },
    noauction: {
        sectionIndex: 8,
        dataFunc: MargoTipsParser.statsParser.noauction
    },
    artisan_worthless: {
        sectionIndex: 8,
        dataFunc: MargoTipsParser.statsParser.artisan_worthless
    },
    soulbound: {
        sectionIndex: 8,
        dataFunc: MargoTipsParser.statsParser.soulbound
    },
    binds: {
        sectionIndex: 8,
        dataFunc: MargoTipsParser.statsParser.binds
    },
    permbound: {
        sectionIndex: 8,
        dataFunc: MargoTipsParser.statsParser.permbound
    },
    leczy: {
        sectionIndex: null,
        dataFunc: MargoTipsParser.statsParser.leczy,
        eqStatClass: true
    },
    legbon_test: {
        sectionIndex: 5,
        dataFunc: MargoTipsParser.statsParser.legbon_test,
        cssClass: 'text-light-green',
        eqStatClass: true
    },
    legbon: {
        sectionIndex: 5,
        dataFunc: MargoTipsParser.statsParser.legbon,
        cssClass: 'text-light-green',
        eqStatClass: true
    },
    ...
    socket_content: {
        sectionIndex: 5,
        dataFunc: MargoTipsParser.statsParser.socket_content,
        eqStatClass: true
    },
    expire_duration: {
        sectionIndex: 1,
        dataFunc: MargoTipsParser.statsParser.expire_duration,
        cssClass: "item-expired"
    },
    action: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.action
    },
    bonus: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.bonus,
        cssClass: 'green-stat'
    },
    lowreq: {
        sectionIndex: 0,
        dataFunc: MargoTipsParser.statsParser.lowreq,
        cssClass: 'upgraded',
        htmlTag: 'b'
    },
    dmg: {
        sectionIndex: 1,
        dataFunc: MargoTipsParser.statsParser.dmg,
        eqStatClass: true
    },

    perheal: {
        sectionIndex: 3,
        dataFunc: MargoTipsParser.statsParser.perheal
    },
    custom_teleport: {
        sectionIndex: 7,
        dataFunc: MargoTipsParser.statsParser.custom_teleport
    },
    wanted_change: {
        sectionIndex: 4,
        dataFunc: MargoTipsParser.statsParser.wanted_change
    },
    ...
    townlimit: {
        sectionIndex: 5,
        tNameActualStatName: {},
        cssClass: "idesc text-light-green",
        htmlTag: "i"
    },
    bonus_not_selected: {
        sectionIndex: 4,
    ...
    recipe: {
        sectionIndex: 3,
        tName: 'recipe_dbl_click'
    },
    upgtimelimit: {
        sectionIndex: 4,
        tName: 'Corectupgtimelimit'
    },
    canpreview: {
        sectionIndex: 8,
        tName: 'canpreviewitem'
    },
    ...
    npc_lootbon: {
        sectionIndex: 3,
        tName: 'npx_lootbon higher %amount%',
        tCategory: "newOrder",
        values: {
            '%amount%': {
                damage: true
    ...
    resmanaendest: {
        ...
        values: {
            ...
            '%val2%': {
                damage: true,
                modify: (x) => Math.max(1, Math.round(x * 0.444))
            }
        },
        cmpStatStr: {
            values: {
